...
(defun get-continuation-byte-p (byte)
  (<= #x80 byte #xbf))

(defun get-valid-utf8-sequence-p (bytes start length)
  (let ((first (aref bytes start)))
    (case length
      (2 (and (<= #xc2 first #xdf)
              (get-continuation-byte-p (aref bytes (1+ start)))))
      (3 (and (get-continuation-byte-p (aref bytes (+ start 2)))
              (or (and (= first #xe0)
                  ...
                  (and (<= #xee first #xef)
                       (<= #x80 (aref bytes (1+ start)) #xbf)))))
      (4 (and (get-continuation-byte-p (aref bytes (+ start 2)))
              (get-continuation-byte-p (aref bytes (+ start 3)))
              (or (and (= first #xf0)
    ...
    (loop while (< index length)
          ...
          do (cond
               ...
               ((< (- length index) sequence-length)
                ...
                (setf index length))
               ((get-valid-utf8-sequence-p input index sequence-length)
                (vector-push-extend
