(in-package #:lispore.utf8)

(defun append-byte (bytes byte)
  (vector-push-extend byte bytes))

(defun encode-code-point (bytes code-point)
  (cond
    ((<= code-point #x7f)
     (append-byte bytes code-point))
    ((<= code-point #x7ff)
     (append-byte bytes (+ #xc0 (ldb (byte 5 6) code-point)))
     (append-byte bytes (+ #x80 (ldb (byte 6 0) code-point))))
    ((<= code-point #xffff)
     (append-byte bytes (+ #xe0 (ldb (byte 4 12) code-point)))
     (append-byte bytes (+ #x80 (ldb (byte 6 6) code-point)))
     (append-byte bytes (+ #x80 (ldb (byte 6 0) code-point))))
    (t
     (append-byte bytes (+ #xf0 (ldb (byte 3 18) code-point)))
     (append-byte bytes (+ #x80 (ldb (byte 6 12) code-point)))
     (append-byte bytes (+ #x80 (ldb (byte 6 6) code-point)))
     (append-byte bytes (+ #x80 (ldb (byte 6 0) code-point))))))

(defun encode-utf8 (text)
  ...
  (let ((bytes (make-array 16
    ...
    (loop for character across text
          ...
          do (if (or (> code-point #x10ffff)
                 ...
                 (encode-code-point bytes #xfffd)
                 (encode-code-point bytes code-point)))
    bytes))
...
(defun decode-code-point (bytes start length)
  (case length
...
(defun decode-utf8-chunk (bytes &optional pending)
  ...
  (let* ((input (if pending
    ...
    (loop while (< index length)
          ...
          for sequence-length = (utf8-sequence-length byte)
          do (cond
               ((= sequence-length 1)
               ...
               ((valid-utf8-sequence-p input index sequence-length)
                (vector-push-extend
                 (code-char (decode-code-point input index sequence-length))
                 text)
